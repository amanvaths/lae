...
export async function resetIndexedAnalytics() {
    const deploy = LAE_MATRIX_DEPLOY_BLOCK;
    const cursor = deploy > 0n ? deploy - 1n : 0n;
    const [laePlacements, matrixSlots, laeIncome, laeUsers, referrals, transactions, clubMatrix, pilotMatrix, income, tokenRewards, withdrawals, spins, stakes, users, chainEvents,] = await prisma.$transaction([

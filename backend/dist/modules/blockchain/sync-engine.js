...
const CONTRACT_TARGETS = [{ key: "matrixCore", address: CONTRACTS.matrixCore, iface: matrixCoreIface }];
let syncing = false;
let pollTimer = null;
function isLogRpcError(err) {
    const msg = err instanceof Error ? err.message : String(err);
    return (/rate limit|limit exceeded|archive requests|too many requests|429|32005|32602/i.test(msg) || /BAD_DATA|UNKNOWN_ERROR/i.test(msg));
}
function syncMode() {
...
async function alignIndexerToDeploy() {
...
async function fetchLogsWithRetry(address, fromBlock, toBlock) {
...
async function syncViaGetLogs(fromBlock, toBlock) {
    let processed = 0;
    ...
    for (const target of CONTRACT_TARGETS) {
        ...
        while (chunkStart <= toBlock) {
            const chunkEnd = chunkStart + batchSize - 1n > toBlock ? toBlock : chunkStart + batchSize - 1n;
...
export async function syncBlockRange(fromBlock, toBlock) {
    ...
    if (mode !== "receipt") {
        try {
            ...
            processed = await syncMatrixReceiptsInRange(Number(fromBlock), Number(toBlock), getIndexerProvider());
        }
    }
    else {
        processed = await syncMatrixReceiptsInRange(Number(fromBlock), Number(toBlock), getIndexerProvider());
        ...
        if (state.lastBlockHash && state.lastBlock > 0n) {
            try {
                ...
                if (stored && stored.hash !== state.lastBlockHash) {
                    const rewind = state.lastBlock > reorgDepth ? state.lastBlock - reorgDepth : startBlock;

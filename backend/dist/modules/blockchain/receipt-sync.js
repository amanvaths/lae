...
async function processBlockReceipts(blockNum, provider) {
    let rawReceipts;
    try {
    ...
    for (const receipt of rawReceipts) {
        for (const log of receipt.logs ?? []) {
            if (log.address.toLowerCase() !== matrixAddress())
                continue;
            try {
...
export async function syncMatrixReceiptsInRange(fromBlock, toBlock, provider = getIndexerProvider()) {
    ...
    for (let cursor = fromBlock; cursor <= toBlock; cursor += SCAN_CONCURRENCY) {
        ...
        const batchEvents = await Promise.all(batch.map((blockNum) => processBlockReceipts(blockNum, provider)));
    ...
    console.log(`[indexer] Receipt scan ${fromBlock}-${toBlock}: ${processed} events (${blocksWithTx} blocks with matrix logs)`);
    return processed;
}
/** Full deploy→head receipt backfill. */
export async function backfillLaeUserEventsFromChain() {
    const fromBlock = Number(process.env.LAE_MATRIX_DEPLOY_BLOCK ??
...
//# sourceMappingURL=receipt-sync.js.map
import { getClubWithdrawAmount, getClubReinvestAmount, CLUB_PACKAGES, } from "../config/packages.js";
...
export async function processClubCycleCompletion(tx, matrixId) {
    ...
    await withIdempotency(tx, idempotencyKey, "CYCLE_COMPLETION", async () => {
        ...
        if (matrix.status === "CYCLE_COMPLETE")
            return null;
        ...
        const hasNextPackage = nextLevel <= CLUB_PACKAGES.length &&
...
export async function processPilotCycleCompletion(tx, matrixId, secondSlotUserId) {
    const idempotencyKey = cycleIdempotencyKey(matrixId);
    await withIdempotency(tx, idempotencyKey, "CYCLE_COMPLETION", async () => {
        ...
        await handlePilotAutoUpgrade(tx, matrix.ownerId, matrix.packageLevel, cyclesCompleted, idempotencyBase, matrix.owner.sponsorId ?? undefined);
...
export async function processPilotFirstSlot(tx, matrixId, slotUserId) {
    const { lockPilotMatrix } = await import("../lib/row-lock.js");
    await lockPilotMatrix(tx, matrixId);

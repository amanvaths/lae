import { DIRECT_REFERRALS_FOR_FIRST_LINE_BONUS, QUALIFIED_REFERRAL_PACKAGE, } from "../config/packages.js";
...
export async function countQualifiedDirectReferrals(tx, userId, matrixType) {
    if (matrixType === "CLUB") {
...
export async function hasCompletedDirectReferrals(tx, userId, matrixType) {
 ...
 */
export async function processFirstLineMemberBonus(tx, userId, matrixType, trigger) {
    ...
    for (const member of firstLineMembers) {
        const pkgLevel = matrixType === "CLUB"
        ...
        if (tokenAmount <= 0)
            continue;
        const idempotencyKey = tokenRewardIdempotencyKey(userId, "FIRST_LINE_BONUS", `${member.id}-${matrixType}-${pkgLevel}-${trigger}`);
...
export async function processFirstLineBonusForSponsorChain(tx, userId, matrixType, trigger) {
...
//# sourceMappingURL=first-line-bonus.engine.js.map
 ...
 */
export async function appendLedgerEntry(tx, input) {
...
export async function creditWallet(tx, input) {
...
export async function debitWithdrawable(tx, userId, amount, idempotencyKey) {
    if (idempotencyKey) {
...
export async function recordIncome(tx, input) {
    await appendLedgerEntry(tx, input);
}
export async function creditTokens(tx, userId, amount, idempotencyKey) {
    if (idempotencyKey) {
...
export async function lockWalletFunds(tx, userId, amount) {
    await lockWallet(tx, userId);
    const wallet = await tx.wallet.findUnique({ where: { userId } });
...
//# sourceMappingURL=income.engine.js.map
...
export async function grantSpinCouponsForQualifiedReferral(tx, sponsorId, referralUserId) {
...
export function pickSpinReward() {
    ...
    for (const entry of SPIN_WEIGHTS) {
        random -= entry.weight;
        if (random <= 0) {
            const amount = SPIN_REWARDS[entry.type === "NO_TOKEN" ? "NO_TOKEN" : entry.type.replace("TOKEN_", "TOKEN_")] ??
...
export async function executeSpin(tx, userId, couponId) {
...
//# sourceMappingURL=spin.engine.js.map
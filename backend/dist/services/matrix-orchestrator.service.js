...
import { purchaseIdempotencyKey, withIdempotency, } from "../lib/idempotency.js";
...
import { distributeWelcomeTokenReward, distributeDirectReferralTokenReward, } from "../engines/token-reward.engine.js";
 ...
 */
export async function executePackagePurchase(input) {
    const amount = input.matrixType === "CLUB"
        ? getClubPackageAmount(input.packageLevel)
        : getPilotPackageAmount(input.packageLevel);
    const idempotencyKey = purchaseIdempotencyKey(input.txHash ?? `${input.userId}-${input.matrixType}-${input.packageLevel}`, input.matrixType, input.packageLevel);
    ...
    await runMatrixTransaction(async (tx) => {
        await withIdempotency(tx, idempotencyKey, "PACKAGE_PURCHASE", async () => {
            ...
            await distributeSponsorPayment(tx, input.userId, input.sponsorId, input.packageLevel, input.matrixType, idempotencyKey);
            await distributeWelcomeTokenReward(tx, input.userId, input.packageLevel, input.matrixType);
            await distributeDirectReferralTokenReward(tx, input.sponsorId, input.userId, input.packageLevel, input.matrixType);
...
export async function executeClubPlacement(tx, userId, sponsorId, packageLevel, idempotencyKey) {
    return placeInClubMatrix(tx, { userId, sponsorId, packageLevel, idempotencyKey });
}
//# sourceMappingURL=matrix-orchestrator.service.js.map
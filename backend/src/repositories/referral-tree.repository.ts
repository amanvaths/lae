 ...
 */
export async function getDownlineRecursive(
  userId: string,
  maxDepth = 10
): Promise<TreeNode[]> {
 ...
 */
export async function getUplineRecursive(
  userId: string,
  maxDepth = 100
): Promise<SponsorChainEntry[]> {
  return prisma.$queryRaw<SponsorChainEntry[]>`
 ...
 */
export function parseUplineFromPath(treePath: string, excludeUserId?: string): string[] {
 ...
 */
export async function getDescendantsByPath(
  userTreePath: string,
  maxDepth?: number
) {
...
export function buildNestedTree(flat: TreeNode[], rootId: string): TreeNode | null {
...
export async function getTeamSize(userId: string, treePath: string): Promise<number> {
...
export async function computeLeaderboard(limit = 100) {
  return prisma.$queryRaw<
    { userId: string; walletAddress: string; totalEarned: string; directCount: bigint; teamSize: bigint }[]
  >`
    SELECT
      u.id AS "userId",
      u.wallet_address AS "walletAddress",
...
export function buildTreePath(sponsorPath: string, userId: string): string {
  const base = sponsorPath.endsWith("/") ? sponsorPath : `${sponsorPath}/`;
  return `${base}${userId}/`;
}

export function computeTreeDepth(treePath: string): number {

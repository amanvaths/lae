...
export const userRepository = {
  ...
  countDirectReferrals(userId: string) {
    return prisma.user.count({ where: { sponsorId: userId, deletedAt: null } });
  },

  getDirectReferrals(userId: string, skip = 0, take = 50) {
  ...
  async getSponsorChain(userId: string, maxDepth = 100) {
    type ChainEntry = { id: string; walletAddress: string; referralCode: string; depth: number };
    const chain: ChainEntry[] = [];
    let currentId: string | null = userId;
    let depth = 0;

    while (currentId && depth < maxDepth) {
      const currentUser: { sponsorId: string | null } | null = await prisma.user.findUnique({
        where: { id: currentId },
        select: { sponsorId: true },
      });
      if (!currentUser?.sponsorId) break;

      const sponsorUser: { id: string; walletAddress: string; referralCode: string } | null =
  ...
  async getReferralTree(userId: string, maxDepth = 5) {
    async function buildTree(id: string, depth: number): Promise<unknown> {
      ...
      const children = await Promise.all(
        user.directReferrals.map((r) => buildTree(r.id, depth + 1))
      );
...
export const walletRepository = {
  ...
  getLedger(userId: string, skip = 0, take = 50) {

...
export async function placeInClubMatrix(
  tx: TransactionClient,
  input: ClubPlacementInput
): Promise<{ matrixId: string; position: import("@prisma/client").MatrixPosition; placementType: import("@prisma/client").PlacementType }> {
  const idempotencyKey =
    input.idempotencyKey ?? placementIdempotencyKey(input.userId, "CLUB", input.packageLevel);
  ...
  let matrixId: string;
  let position: import("@prisma/client").MatrixPosition;
  let placementType: import("@prisma/client").PlacementType = input.placementType ?? "DIRECT";
  let spilloverFrom: string | null = null;
...
export async function createClubMatrixForOwner(
  tx: TransactionClient,
  ownerId: string,
  packageLevel: number,
  isRebirth = false,
...
async function logMatrixOperation(
  tx: TransactionClient,
  userId: string,

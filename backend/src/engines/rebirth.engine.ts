...
export async function createClubRebirth(
  tx: TransactionClient,
  ownerId: string,
  packageLevel: number,
  parentMatrixId: string,
  _reinvestAmount: number
): Promise<string> {
  ...
  return withIdempotency(tx, idempotencyKey, "REBIRTH_CREATION", async () => {
    const rebirthMatrixId = await createClubMatrixForOwner(
      tx,
      ownerId,
      packageLevel,
      true,
...
export async function createPilotRebirth(
  tx: TransactionClient,
  ownerId: string,
  packageLevel: number,
  parentMatrixId: string

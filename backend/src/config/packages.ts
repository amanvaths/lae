...
export const PILOT_POSITIONS = ["SLOT_1", "SLOT_2"] as const;

export function getClubPackageAmount(level: number): number {
  const amount = CLUB_PACKAGES[level - 1];
  if (!amount) throw new Error(`Invalid club package level: ${level}`);
  return amount;
}

export function getPilotPackageAmount(level: number): number {
  const amount = PILOT_PACKAGES[level - 1];
  if (!amount) throw new Error(`Invalid pilot package level: ${level}`);
  return amount;
}

export function getPilotPoolAmount(level: number): number {
  const amount = PILOT_POOL_AMOUNTS[level - 1];
  if (!amount) throw new Error(`Invalid pilot package level: ${level}`);
  return amount;
}

export function getClubCycleReward(packageLevel: number): number {
  return getClubPackageAmount(packageLevel) * CLUB_CYCLE_MULTIPLIER;
}

export function getClubWithdrawAmount(packageLevel: number): number {
  return getClubCycleReward(packageLevel) * CLUB_WITHDRAW_RATIO;
}

export function getClubReinvestAmount(packageLevel: number): number {

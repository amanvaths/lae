...
async function processReceiptLogs(
  ...
  for (const raw of receipt.logs) {
    if (raw.address.toLowerCase() !== matrixAddress()) continue;
    try {
...
async function shouldSkipReceiptScan(toBlock: number): Promise<boolean> {
  if (process.env.FORCE_EVENT_BACKFILL === "1") return false;
  ...
  if (incomeCount === 0) return false;
  if (!state?.lastBlock) return false;
...
export async function backfillLaeUserEventsFromChain(): Promise<number> {
  ...
  for (let blockNum = fromBlock; blockNum <= toBlock; blockNum++) {
    ...
    for (const tx of txs) {
      if (tx.to?.toLowerCase() !== matrixAddress()) continue;

      txCount++;
      try {
        const receipt = await provider.getTransactionReceipt(tx.hash);
        if (receipt) processed += await processReceiptLogs(receipt, iface);
      } catch (err) {
  ...
  console.log(
    `[backfill] Receipt scan: ${processed} events from ${txCount} matrix txs (blocks ${fromBlock}-${toBlock}, incomeRows=${incomeCount})`
  );
  return processed;
}

...
export function startDepositListener(contractAddress: string): void {
  ...
  contract.on("Deposit", async (user: string, amount: bigint, matrixType: number, packageLevel: number, _txRef: string, event: ethers.EventLog) => {
    try {

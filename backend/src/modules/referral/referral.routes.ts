...
import {
  ...
  computeLeaderboard,
  getTeamSize,
} from "../../repositories/referral-tree.repository.js";
import {
  getCached,
  setCached,
  CACHE_KEYS,
...
export async function referralRoutes(app: FastifyInstance): Promise<void> {
  app.get("/tree", {
    ...
    const depth = Number((request.query as { depth?: string }).depth ?? 5);
  ...
  app.get("/tree/recursive", {
    ...
    const depth = Number((request.query as { depth?: string }).depth ?? 10);
...
export async function cacheRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/leaderboard", {
    ...
    const cached = await getCached<{ entries: unknown[]; cached: boolean }>(cacheKey);
